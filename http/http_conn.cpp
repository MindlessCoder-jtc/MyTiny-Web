...
/*
    在HTTP报文中，每一行的数据由\r\n作为结束字符，空行则是仅仅是字符\r\n，因此，可以通过查找\r\n将报文
    拆解成单独的行进行解析
    从状态机负责读取buffer中的数据，将每行数据末尾的\r\n置为\0\0,并更新从状态机在buffer中读取的
    位置m_checked_idx,以此来驱动主状态机解析
*/
...
bool http_conn::read_once()
{
    ...
    else
    {
        while (true)
        {
            ...
            {
                 /*
                    对于非阻塞IO，下面的条件成立表示数据已经全部读取完毕，此后，epoll就能再次触发
...
//HTTP 请求报文的请求行 GET http://wwww.baidu.com/index.html HTTP/1.1
...
http_conn::HTTP_CODE http_conn::parse_headers(char *text)
{
    ...
    if (text[0] == '\0')
    {
        //是空行，判断content-length是否为０
        if (m_content_length != 0)
        {
            //不是０，是post请求，里面包含客户端传来的表单信息
...
{   
    //判断buffer中是否读取了消息体， m_checked_idx是parse_line解析一行后缓冲区为解析的位置
...
http_conn::HTTP_CODE http_conn::process_read()
{
    ...
    */
    ...
    //如果状态机转移到CHECK_STATE_CONTENT并且完整读取一行 或 从状态机转移到LINE_OK,继续循环
    /*
        用parse_line读取报文的每一行
            返回值：
                LINE_OPEN: 需要继续接收数据
    ...
    */
    while ((m_check_state == CHECK_STATE_CONTENT && line_status == LINE_OK) || ((line_status = parse_line()) == LINE_OK))
    {
        /*
            m_check_state 状态变化：
                CHECK_STATE_REQUESTLINE（解析请求行） -> CHECK_STATE_HEADER（解析首部信息） -> CHECK_STATE_CONTENT（解析实体体）
        */
        text = get_line(); //text存储刚刚parse_line读取的一行报文
        //m_start_line是每一个数据行在m_read_buf中的起始位置
        //m_checked_idx表示从状态机在m_read_buf中读取的位置
        m_start_line = m_checked_idx;// 更新新的数据行的起始位置
        LOG_INFO("%s", text);
        //主状态机的三种状态转移逻辑
...
{
    /*
        从打开网站到新用户注册\登录m_url变换：
            /judeg.html -> /0 -> /3CGISQL.cgi -> /2CGISQL.cgi
    */
...
bool http_conn::write()
{
    ...
    while (1)
    {
        ...
        if (bytes_have_send >= m_iv[0].iov_len)
        {
            //不再继续发送头部信息
            m_iv[0].iov_len = 0;
            m_iv[1].iov_base = m_file_address + (bytes_have_send - m_write_idx); //m_write_idx是第一个缓冲区的大小
...
bool http_conn::process_write(HTTP_CODE ret)
{
    ...
    {
        add_status_line(200, ok_200_title);//如果请求的资源存在
...
//各子线程通过函数对任务进行处理，调用process_read函数和process_write函数分别完成报文解析与报文响应
